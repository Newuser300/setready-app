...
export default function ProfileImportPage() {
  ...
  // tracks which field names were last filled by AI (cleared when user edits)
  ...
  // Remove a field from the AI-suggested set when the user edits it manually
  function clearAi(...fields: string[]) {
    setAiFields(prev => {
  ...
  // Return amber-highlighted input style when the field was AI-filled
  function aiInp(field: string): React.CSSProperties {
    return aiFields.has(field)
      ? { ...inp, border: '1.5px solid #F59E0B', backgroundColor: '#fffbeb' }
      : inp
  }

  // Small "AI" badge shown next to the label when the field was AI-filled
  function AiBadge({ field }: { field: string }) {
    if (!aiFields.has(field)) return null
    return (
      <span style={{
        fontSize: '10px', color: '#92400e', backgroundColor: '#fef3c7',
        border: '1px solid #fde68a', borderRadius: '4px',
        padding: '1px 6px', fontWeight: 700, marginLeft: '6px',
        verticalAlign: 'middle', letterSpacing: '0.04em',
      }}>AI</span>
    )
  }

  // Inline "Verify" note shown below high-risk AI-filled fields
  function VerifyNote({ field, message }: { field: string; message: string }) {
    if (!aiFields.has(field)) return null
    return (
      <p style={{
        margin: '5px 0 0', fontSize: '11px', color: '#b45309',
  ...
  const handleAiParse = useCallback(async () => {
    ...
    try {
      ...
      if (p.gender)              { setGender(p.gender);                     filled.add('gender') }
      if (p.date_of_birth)       { setDob(p.date_of_birth);                 filled.add('dob') }
      if (p.height_cm)           { setHeightCm(String(p.height_cm));        filled.add('height_cm') }
      if (p.hair_color)          { setHairColor(p.hair_color);              filled.add('hair_color') }
      if (p.eye_color)           { setEyeColor(p.eye_color);                filled.add('eye_color') }
      if (p.body_type)           { setBodyType(p.body_type);                filled.add('body_type') }
      if (p.ethnicity)           { setEthnicity(p.ethnicity);               filled.add('ethnicity') }
      if (p.union_status)        { setUnionStatus(p.union_status);          filled.add('union_status') }
      if (p.special_skills?.length) { setSkills(p.special_skills.join(', ')); filled.add('skills') }
      if (p.languages?.length)   { setLanguages(p.languages.join(', '));    filled.add('languages') }
      if (p.accents?.length)     { setAccents(p.accents.join(', '));        filled.add('accents') }
      if (p.acting_experience)   { setActingExperience(p.acting_experience); filled.add('acting_experience') }
      if (p.bio)                 { setBio(p.bio);                           filled.add('bio') }
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f9fafb', fontFamily: '-apple-system, Arial, sans-serif' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '28px 20px 80px' }}>
        ...
        {aiEnabled && (
          <div style={{ ...card, backgroundColor: '#fffbeb', border: '1px solid #fde68a' }}>
            ...
            <textarea
              ...
              style={{ ...inp, minHeight: '100px', resize: 'vertical', lineHeight: '1.5', marginBottom: '10px', backgroundColor: 'white', border: '1px solid #fde68a' }}
            />
            {aiError && (
        ...
        {/* ── AI DISCLAIMER BANNER — shown immediately after parse ──────── */}
        {aiDraft && (
          <div style={{
            ...
            <div>
              <div style={{ fontSize: '13px', fontWeight: 700, color: '#78350f', marginBottom: '4px' }}>
                Review every AI-suggested field before saving
              </div>
              <div style={{ fontSize: '13px', color: '#92400e', lineHeight: '1.55' }}>
                Fields highlighted in amber below were extracted by AI and may contain errors. Double-check your stats, union status, and skills.{' '}
                <strong>You are responsible for the accuracy of your profile.</strong>
              </div>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSave}>
          ...
          <div style={card}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                <label style={lbl}>
                  Gender
                  <AiBadge field="gender" />
                </label>
                <select
                  style={{ ...aiInp('gender'), cursor: 'pointer' }}
                  value={gender}
                  onChange={e => { setGender(e.target.value); clearAi('gender') }}
                >
              ...
              <div>
                ...
                <input
                  style={aiInp('dob')}
                  value={dob}
                  onChange={e => { setDob(e.target.value); clearAi('dob') }}
                  type="date"
                />
                <VerifyNote field="dob" message="Verify this is correct — wrong DOB causes booking problems" />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                <label style={lbl}>
                  Height (cm)
                  <AiBadge field="height_cm" />
                </label>
                <input
                  style={aiInp('height_cm')}
                  value={heightCm}
                  onChange={e => { setHeightCm(e.target.value); clearAi('height_cm') }}
                  placeholder="168" type="number" min="100" max="250"
                />
              </div>
              <div>
                <label style={lbl}>
                  Hair Color
                  <AiBadge field="hair_color" />
                </label>
                <input
                  style={aiInp('hair_color')}
                  value={hairColor}
                  onChange={e => { setHairColor(e.target.value); clearAi('hair_color') }}
                  placeholder="Brown"
                />
              </div>
              <div>
                <label style={lbl}>
                  Eye Color
                  <AiBadge field="eye_color" />
                </label>
                <input
                  style={aiInp('eye_color')}
                  value={eyeColor}
                  onChange={e => { setEyeColor(e.target.value); clearAi('eye_color') }}
                  placeholder="Blue"
                />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div>
                <label style={lbl}>
                  Body Type
                  <AiBadge field="body_type" />
                  <span style={{ color: '#9ca3af', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}> (optional)</span>
                </label>
                <input
                  style={aiInp('body_type')}
                  value={bodyType}
                  onChange={e => { setBodyType(e.target.value); clearAi('body_type') }}
                  placeholder="Athletic, slim, average…"
                />
              </div>
              <div>
                <label style={lbl}>
                  Ethnicity
                  <AiBadge field="ethnicity" />
                  <span style={{ color: '#9ca3af', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}> (optional)</span>
                </label>
                <input
                  style={aiInp('ethnicity')}
                  value={ethnicity}
                  onChange={e => { setEthnicity(e.target.value); clearAi('ethnicity') }}
          ...
          <div style={card}>
            ...
            <div style={{ marginBottom: '12px' }}>
              <label style={lbl}>
                Union Status
                <AiBadge field="union_status" />
              </label>
              <select
                style={{ ...aiInp('union_status'), cursor: 'pointer' }}
                value={unionStatus}
                onChange={e => { setUnionStatus(e.target.value); clearAi('union_status') }}
              >
                <option value="">Select…</option>
                {UNION_OPTIONS.map(u => <option key={u.value} value={u.value}>{u.label}</option>)}
              </select>
              <VerifyNote field="union_status" message="Verify this is correct — wrong union status causes booking problems" />
            </div>

            <div style={{ marginBottom: '12px' }}>
              <label style={lbl}>
                Special Skills
                <AiBadge field="skills" />
                <span style={{ color: '#9ca3af', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}> (comma-separated)</span>
              </label>
              <input
                style={aiInp('skills')}
                value={skills}
                onChange={e => { setSkills(e.target.value); clearAi('skills') }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div>
                <label style={lbl}>
                  Languages
                  <AiBadge field="languages" />
                  <span style={{ color: '#9ca3af', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}> (comma-separated)</span>
                </label>
                <input
                  style={aiInp('languages')}
                  value={languages}
                  onChange={e => { setLanguages(e.target.value); clearAi('languages') }}
              ...
              <div>
                <label style={lbl}>
                  Accents
                  <AiBadge field="accents" />
                  <span style={{ color: '#9ca3af', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}> (comma-separated)</span>
                </label>
                <input
                  style={aiInp('accents')}
                  value={accents}
                  onChange={e => { setAccents(e.target.value); clearAi('accents') }}
                  placeholder="British RP, Southern…"
                />
              </div>
            </div>
            ...
            <div style={{ marginBottom: '12px' }}>
              <label style={lbl}>
                Acting Experience
                <AiBadge field="acting_experience" />
                <span style={{ color: '#9ca3af', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}> (optional)</span>
              </label>
              <input
                style={aiInp('acting_experience')}
                value={actingExperience}
                onChange={e => { setActingExperience(e.target.value); clearAi('acting_experience') }}
                placeholder="e.g. 5 years background, beginner, 10+ years film/TV"
              />
            </div>
            ...
            <div>
              <label style={lbl}>
                Bio
                <AiBadge field="bio" />
                <span style={{ color: '#9ca3af', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}> (1–3 sentences)</span>
              </label>
              <textarea
                style={{ ...aiInp('bio'), resize: 'vertical', minHeight: '90px', lineHeight: '1.55' }}
                value={bio}
                onChange={e => { setBio(e.target.value); clearAi('bio') }}
            ...
            {headshotPreview ? (
              ...
              <div
                ...
                style={{
                  ...
                  cursor: 'pointer', backgroundColor: draggingHeadshot ? '#fffbeb' : '#fafafa',
          ...
          >
            {saving ? 'Saving…' : aiDraft ? 'Review & Save My Profile' : 'Save My Profile'}
          </button>
        </form>
      </div>
    </div>
