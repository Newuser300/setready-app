    ...
    if (entityType === 'casting_director') {
      ...
      if (cd?.email) {
        await sendEmail({
          ...
          html: `<p>Hi ${cd.name || 'there'},</p><p>After review, we are unable to approve your casting director application at this time.${reason ? ` Reason: ${reason}` : ''}</p><p>If you have questions, please contact us at support@setready.site.</p><p>— The BGReady Team</p>`,
        }).catch(() => {})
      }
    } else if (entityType === 'agency') {
      ...
      if (ag?.email) {
        await sendEmail({
          ...
          html: `<p>Hi ${ag.contact_name || 'there'},</p><p>After review, we are unable to approve your agency application for <strong>${ag.name}</strong> at this time.${reason ? ` Reason: ${reason}` : ''}</p><p>If you have questions, please contact us at support@setready.site.</p><p>— The BGReady Team</p>`,
        }).catch(() => {})
      }
    } else if (entityType === 'casting_request') {
      const { data: cr } = await supabaseAdmin
      ...
      if (cdEmail) {
        await sendEmail({
          ...
          html: `<p>Hi ${cdName || 'there'},</p><p>Your casting request for <strong>${cr?.production_name}</strong> was not approved at this time.${reason ? ` Reason: ${reason}` : ''}</p><p>Please contact us at support@setready.site if you have questions.</p><p>— The BGReady Team</p>`,
        }).catch(() => {})
      }
    }
