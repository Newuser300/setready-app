...
const ITEMS: Record<string, { priceEnv: string }> = {
  ...
  skip:    { priceEnv: 'STRIPE_TYCOON_SKIP_PRICE_ID' },
  auto:    { priceEnv: 'STRIPE_TYCOON_AUTO_PRICE_ID' },
  offline: { priceEnv: 'STRIPE_TYCOON_OFFLINE_PRICE_ID' },
};
...
export async function POST(request: Request) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${appUrl}/games/studio-tycoon?purchase=success`,
      cancel_url: `${appUrl}/games/studio-tycoon?purchase=cancelled`,
      client_reference_id: user.id,
      ...abandonedCartOptions({ email: user.email, mode: 'payment' }),
      metadata: { userId: user.id, type: 'game_purchase', game: 'studio-tycoon', item, itemName: 'Studio Tycoon item', returnPath: '/games/studio-tycoon' },
    });

    return NextResponse.json({ url: session.url });
  } catch (error) {
    console.error('Studio Tycoon checkout error:', error);
