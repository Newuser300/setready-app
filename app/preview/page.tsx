...
function LockedCard({
  ...
  return (
    <button
      onClick={onUnlock}
      className={`group w-full text-left bg-[#1A1A1A] rounded-2xl p-6 relative
        border-t-2 ${isGold ? 'border-amber-500' : 'border-gray-500'}
        hover:brightness-110 transition-all duration-200
        ${isGold
          ? 'hover:shadow-lg hover:shadow-amber-500/10 hover:border-amber-400'
          : 'hover:shadow-lg hover:shadow-gray-400/10 hover:border-gray-400'
        }`}
    >
      {/* Lock icon */}
      <span className={`absolute top-4 right-4 text-base ${isGold ? 'text-amber-500' : 'text-gray-500'}`}>
        🔒
      </span>

      {/* Module number badge */}
      <span className={`inline-block text-xs font-bold px-2 py-0.5 rounded-md mb-3 ${
        isGold ? 'bg-amber-500/15 text-amber-400' : 'bg-gray-600/25 text-gray-400'
      }`}>
        {num}
      </span>

      {/* Title */}
      <h3 className="text-white font-bold text-base mb-2 pr-6 leading-snug">{title}</h3>

      {/* Description */}
      <p className="text-gray-400 text-sm leading-relaxed mb-4">{desc}</p>

      {/* Footer */}
      <p className={`text-xs font-medium ${isGold ? 'text-amber-600' : 'text-gray-600'}`}>
        🔒 Sign up to access
      </p>
    </button>
  );
...
export default function PreviewPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      {/* ── STICKY TOP BANNER ───────────────────────────────── */}
      <div className="fixed top-0 left-0 right-0 z-50 bg-[#0A0A0A] border-b border-white/10 shadow-md">
      ...
      <div className="pt-[52px]">
        ...
        {/* ── HERO AREA ───────────────────────────────────────── */}
        <section className="bg-[#0A0A0A] border-b border-white/8 px-4 py-14 sm:py-20">
          <div className="max-w-6xl mx-auto">
            <FadeIn>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-amber-500/25 bg-amber-500/8 text-amber-400 text-xs font-semibold mb-6">
                👁 Preview Mode — Explore the full curriculum
              </div>
              <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold text-white mb-5 leading-tight max-w-3xl">
              ...
              <div className="max-w-sm">
                ...
                <p className="text-xs text-gray-600 mt-2">Sign up to start tracking your progress</p>
              </div>
            </FadeIn>
          </div>
        </section>
            ...
            <FadeIn>
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 pl-4 border-l-4 border-amber-500">
                <div>
                  ...
                  <p className="text-gray-400 text-sm mt-1">$9.99/month • Cancel anytime</p>
                </div>
                <button
                  onClick={() => setShowModal(true)}
                  className="shrink-0 px-6 py-2.5 bg-amber-500 text-black font-bold rounded-xl hover:bg-amber-400 hover:scale-105 transition-all duration-200 text-sm shadow-lg shadow-amber-500/20"
                >
                  Unlock Full Access →
                </button>
              </div>
        ...
        {/* ── SECTION 2 PREVIEW ───────────────────────────────── */}
        <section className="bg-[#111111] px-4 py-14 border-y border-white/6">
          <div className="max-w-6xl mx-auto">

            {/* Section header */}
            <FadeIn>
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 pl-4 border-l-4 border-gray-500">
                <div>
                  <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-gray-700/50 text-gray-300 text-xs font-semibold rounded mb-2 uppercase tracking-wide">
                    Advanced
                  </div>
                  <h2 className="text-xl sm:text-2xl font-bold text-white">
                    Section 2: Advanced Acting Techniques
                  </h2>
                  <p className="text-gray-400 text-sm mt-1">$19.99 one-time purchase</p>
                </div>
                <button
                  onClick={() => setShowModal(true)}
                  className="shrink-0 px-6 py-2.5 bg-transparent text-white border border-white/25 font-bold rounded-xl hover:bg-white/8 hover:border-white/50 transition-all duration-200 text-sm"
                >
                  Unlock Section 2 →
                </button>
              </div>
        ...
        <section className="px-4 py-14">
          <div className="max-w-6xl mx-auto">
            <FadeIn>
              <div className="mb-8">
                <h2 className="text-xl font-bold text-white mb-1">More Tools Included</h2>
                <p className="text-gray-400 text-sm">Everything you need to manage your career, all in one place</p>
              </div>
            </FadeIn>
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory sm:grid sm:grid-cols-3 sm:overflow-visible sm:pb-0">
              {[
                ...
                <FadeIn key={f.title} delay={i * 80} className="snap-start shrink-0 w-72 sm:w-auto">
                  <button
                    onClick={() => setShowModal(true)}
                    className="w-full h-full text-left bg-[#1A1A1A] rounded-2xl p-6 border border-white/5 hover:border-amber-500/20 transition-all duration-200 group"
                  >
                    <div className="flex justify-between items-start mb-4">
                      <span className="text-3xl">{f.icon}</span>
                      <span className="text-amber-500/70 text-sm group-hover:text-amber-400 transition">🔒</span>
                    </div>
                    <h3 className="text-white font-bold mb-2">{f.title}</h3>
                    <p className="text-gray-400 text-sm leading-relaxed mb-4">{f.desc}</p>
                    <p className="text-xs text-gray-600 group-hover:text-amber-600 transition">
                      Sign up to access →
                    </p>
                  </button>
                </FadeIn>
        ...
        {/* ── BOTTOM CTA ──────────────────────────────────────── */}
        <section className="bg-[#111111] border-t border-white/8 px-4 py-20">
          <FadeIn>
            <div className="max-w-2xl mx-auto text-center">
              ...
              <button
                onClick={() => setShowModal(true)}
                className="px-10 py-4 bg-amber-500 text-black font-bold text-lg rounded-xl hover:bg-amber-400 hover:scale-105 transition-all duration-200 shadow-xl shadow-amber-500/20 mb-4"
              >
                Create Free Account
              </button>
